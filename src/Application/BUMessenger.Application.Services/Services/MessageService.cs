    ...
    ILogger<MessageService> logger) : IMessageService
{
    ...
    public async Task<Paged<Message>> GetMessagesAsync(Guid chatId, Guid userId, PageFilters filters)
    {
        try
        {
            if (!await _chatRepository.UserInChatAsync(chatId, userId))
            {
                _logger.LogInformation("User with id = {Id} isn't in chat with id = {ChatId}.", userId, chatId);
                throw new UserNotInChatServiceException($"User with id = {userId} isn't in chat with id = {chatId}.");
            }
    ...
    public async Task<Paged<Message>> GetThreadMessagesAsync(Guid chatId,
        Guid userId,
